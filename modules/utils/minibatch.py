...
class MyDataset(Dataset):
    def __init__(self, graphs, features, adjs, context_pairs, time_steps):
        super(MyDataset, self).__init__()
        # some args value is set as the same as the default value in DySAT PyTorch
        self.window = -1
        ...
        self.max_positive = self.neg_sample_size
        self.train_nodes = list(self.graphs[self.time_steps-1].nodes()) # all nodes in the graph.
        self.min_t = max(self.time_steps - self.window - 1, 0) if self.window > 0 else 0
    ...
    def __createitems__(self):
        ...
        for node in list(self.graphs[self.time_steps-1].nodes()):
            ...
            node_2_negative = [np.array(node) for node in node_2_negative] #Tansfer to numpy.array
...
"""
Copy from utilities in DySAT PyTorch
"""
