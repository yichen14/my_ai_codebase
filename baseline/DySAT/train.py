...
from utils.preprocess import load_graphs_new
...
def train(args, dataloader, model, opt, train_edges_pos, train_edges_neg, val_edges_pos, val_edges_neg, test_edges_pos, test_edges_neg):
    # in training
    best_epoch_val = 0
    patient = 0
    ...
    for epoch in range(args.epochs):
        ...
        print("Epoch {:<3},  Loss = {:.3f}, Val AUC {:.3f} Test AUC {:.3f}; Val AP {:.3f} Test AP {:.3f}".format(epoch, 
                                                                np.mean(epoch_loss),
                                                                epoch_auc_val, 
                                                                epoch_auc_test,
                                                                epoch_ap_val,
    ...
    ap_val = val_results["HAD"][1]
    ap_test = test_results["HAD"][1]
    print("Best Test AUC = {:.3f}, Best Test AP = {:.3f}".format(auc_test, ap_test))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--early_stop", type=int, default=20, #(default:10)
                        help="patient")
    ...
    for _ in range(NUM_TESTS):
        auc_best, ap_best = train(args, dataloader, model, opt, train_edges_pos, train_edges_neg, val_edges_pos, val_edges_neg, test_edges_pos, test_edges_neg)
        auc_test.append(auc_best)
        ap_test.append(ap_best)
    
    print("FINAL SCORE:")
    print("AUC Mean {:.4f}, SD {:.4f}".format(mean(auc_test)*100, stdev(auc_test)*100))
    print("AP Mean {:.4f}, SD {:.4f}".format(mean(ap_test)*100, stdev(ap_test)*100))
    
    outf = 'DySAT'
    f = open(outf, 'a')
    f.write(args.dataset + '\n')
    f.write('===== ptb_rate %0.4f; test_len %i =====\n' % (args.att, args.test_len))
    f.write("AUC Mean {:.4f}, SD {:.4f}\n ".format(mean(auc_test)*100, stdev(auc_test)*100))
    f.write("AP Mean {:.4f}, SD {:.4f} \n\n".format(mean(ap_test)*100, stdev(ap_test)*100)) 
    f.close()
