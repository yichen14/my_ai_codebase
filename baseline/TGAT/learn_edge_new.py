...
parser.add_argument('--bs', type=int, default=64, help='batch_size') #original default: 200
...
g_df = pd.read_csv('./processed/ml_{}.csv'.format(DATA)) # df(u, i, ts, label, idx)
# e_feat = np.load('./processed/ml_{}.npy'.format(DATA)) # edge feature (num_edges+1, edge_feat_dim) (672448, 172)
n_feat = np.load('./processed/ml_{}_node.npy'.format(DATA)) # node feature zeros(max_node, edge_feat_dim) (10985, 172) num_nodes=11,000

# val_time, test_time = list(np.quantile(g_df.ts, [0.70, 0.85])) # 1882469.2648, 2261813.6575999996
test_len = 3
time_max, time_min = g_df.ts.max(), g_df.ts.min()
val_time, test_time = time_max - test_len, time_max - test_len + 1 #10-3+1=8, 9
...
num_total_unique_nodes = len(total_node_set) #10984
...
# print(train_rand_sampler.sample(10))
# exit()

### Model initialize
device = torch.device('cuda:{}'.format(GPU))
tgan = TGAN(train_ngh_finder, n_feat,
